...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (seedBtn) seedBtn.addEventListener('click', async function() {
    var log = document.getElementById('seedLog');
    log.style.display = 'block';
    log.textContent = 'Re-embedding...\n';
  ...
  if (searchBtn) searchBtn.addEventListener('click', async function() {
    ...
    el.innerHTML = '<span class="spin"></span> Searching...';
    try {
      var r = await fetch('/chat', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({message:q, state:'qa'})});
      var d = await r.json();
      if (!d.matches || !d.matches.length) { el.innerHTML = '<p style="color:var(--muted)">No matches.</p>'; return; }
      el.innerHTML = d.matches.map(function(m) {
        return '<div style="background:rgba(255,255,255,.04);border:1px solid var(--border);border-radius:var(--r);padding:1rem;margin-bottom:.75rem">'
          + '<div style="font-size:.72rem;color:var(--a);font-weight:600;margin-bottom:.3rem">' + (m.score*100).toFixed(1) + '%</div>'
          + '<div style="font-size:.9rem;font-weight:600;color:#fff;margin-bottom:.25rem">' + m.title + '</div>'
          + '<div style="font-size:.78rem;color:var(--muted)">' + m.summary + '</div></div>';
      }).join('');
    } catch(e) { el.innerHTML = '<p style="color:#f87171">' + e.message + '</p>'; }
  });
  var searchQ = document.getElementById('searchQ');
  if (searchQ) searchQ.addEventListener('keydown', function(e) { if (e.key === 'Enter') document.getElementById('searchBtn').click(); });
  ...
  if (publishBtn) publishBtn.addEventListener('click', async function() {
    ...
    try {
      var r = await fetch('/api/publish', {method:'POST'}); var d = await r.json();
      if (d.ok) { btn.textContent = 'Published!'; btn.style.background = '#22c55e'; setTimeout(function() { btn.textContent = 'Publish Site Live'; btn.style.background = ''; btn.disabled = false; }, 3000); loadStatus(); }
      else { btn.textContent = 'Error: ' + (d.error || 'unknown'); btn.disabled = false; }
...
function leadStatusBadge(s) {
  var map = {"new":"#3b82f6","contacted":"#f59e0b","quoted":"#8b5cf6","won":"#22c55e","lost":"#ef4444"};
  var c = map[s] || "#6b7280";
  return '<span style="background:' + c + ';color:#fff;font-size:.68rem;font-weight:600;padding:.15rem .55rem;border-radius:10px;text-transform:uppercase">' + s + '</span>';
}
window.leadStatusBadge = leadStatusBadge;

function cbStatusBadge(s) {
  var map = {"pending":"#f59e0b","called":"#22c55e","no_answer":"#ef4444","scheduled":"#8b5cf6"};
  var c = map[s] || "#6b7280";
  return '<span style="background:' + c + ';color:#fff;font-size:.68rem;font-weight:600;padding:.15rem .55rem;border-radius:10px;text-transform:uppercase">' + s + '</span>';
}
window.cbStatusBadge = cbStatusBadge;

async function loadLeads() {
  var el = document.getElementById('leadsTable');
  try {
    var r = await fetch('/api/admin/leads'); var d = await r.json();
    if (!d.ok || !d.leads || !d.leads.length) { el.innerHTML = '<p style="color:var(--muted)">No leads yet.</p>'; return; }
    var html = '<div style="margin-bottom:.75rem"><a href="/api/admin/leads?format=csv" class="btn btn-outline btn-sm" download>Export CSV</a></div>'
      + '<div style="overflow-x:auto"><table><thead><tr><th>Name</th><th>Email</th><th>Phone</th><th>Service</th><th>Budget</th><th>Timeline</th><th>Section</th><th>Status</th><th>Date</th></tr></thead><tbody>';
    d.leads.forEach(function(l) {
      html += '<tr><td>' + l.name + '</td><td>' + l.email + '</td><td>' + l.phone + '</td><td>' + (l.service||'') + '</td><td>' + (l.budget_range||'') + '</td><td>' + (l.timeline||'') + '</td>'
        + '<td style="font-size:.75rem;color:var(--a)">' + (l.lead_section||'') + '</td>'
        + '<td>' + leadStatusBadge(l.status||'new')
        + '<br><select onchange="updateLeadStatus(' + l.id + ',this.value)" style="margin-top:.3rem;background:rgba(255,255,255,.08);border:1px solid var(--border);color:#fff;font-size:.72rem;padding:.2rem .4rem;border-radius:4px;outline:none">'
        + ['new','contacted','quoted','won','lost'].map(function(v) { return '<option value="' + v + '"' + (l.status===v?' selected':'') + '>' + v + '</option>'; }).join('')
        + '</select></td><td>' + (l.created_at||'').slice(0,16) + '</td></tr>';
    });
...
async function updateLeadStatus(id, status) {
  try { await fetch('/api/admin/leads/' + id, {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({status:status})}); }
  catch(e) { alert(e.message); }
}
...
async function loadCallbacks() {
  ...
  try {
    var r = await fetch('/api/admin/callbacks'); var d = await r.json();
    if (!d.ok || !d.callbacks || !d.callbacks.length) { el.innerHTML = '<p style="color:var(--muted)">No callbacks yet.</p>'; return; }
    var html = '<div style="margin-bottom:.75rem"><a href="/api/admin/callbacks?format=csv" class="btn btn-outline btn-sm" download>Export CSV</a></div>'
      + '<div style="overflow-x:auto"><table><thead><tr><th>Name</th><th>Phone</th><th>Time</th><th>Service</th><th>Section</th><th>Status</th><th>Date</th></tr></thead><tbody>';
    d.callbacks.forEach(function(c) {
      html += '<tr><td>' + c.name + '</td><td>' + c.phone + '</td><td>' + (c.preferred_time||'') + '</td><td>' + (c.service||'') + '</td>'
        + '<td style="font-size:.75rem;color:var(--a)">' + (c.lead_section||'') + '</td>'
        + '<td>' + cbStatusBadge(c.status||'pending')
        + '<br><select onchange="updateCallbackStatus(' + c.id + ',this.value)" style="margin-top:.3rem;background:rgba(255,255,255,.08);border:1px solid var(--border);color:#fff;font-size:.72rem;padding:.2rem .4rem;border-radius:4px;outline:none">'
        + ['pending','called','no_answer','scheduled'].map(function(v) { return '<option value="' + v + '"' + (c.status===v?' selected':'') + '>' + v + '</option>'; }).join('')
        + '</select></td><td>' + (c.created_at||'').slice(0,16) + '</td></tr>';
    });
    html += '</tbody></table></div>';
    el.innerHTML = html;
  } catch(e) { el.innerHTML = '<p style="color:#f87171">' + e.message + '</p>'; }
}
window.loadCallbacks = loadCallbacks;

async function updateCallbackStatus(id, status) {
  try { await fetch('/api/admin/callbacks/' + id, {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({status:status})}); }
  catch(e) { alert(e.message); }
}
window.updateCallbackStatus = updateCallbackStatus;

// ── Articles ──────────────────────────────────────────────────────────────────
var editingArticleId = null;

async function loadArticles() {
  var el = document.getElementById('articlesPanel');
  try {
    var r = await fetch('/api/admin/articles'); var d = await r.json();
    var html = '<div style="margin-bottom:1rem;display:flex;gap:.75rem;align-items:center">'
      + '<button class="btn btn-gold btn-sm" onclick="openArticleEditor(null)">+ New Article</button>'
      + '<span style="font-size:.8rem;color:var(--muted)">' + (d.articles ? d.articles.length : 0) + ' articles</span></div>';
    if (!d.articles || !d.articles.length) { el.innerHTML = html + '<p style="color:var(--muted)">No articles yet.</p>'; return; }
    html += '<div style="overflow-x:auto"><table><thead><tr><th>Title</th><th>Status</th><th>Date</th><th>Actions</th></tr></thead><tbody>';
    d.articles.forEach(function(a) {
      var pub = a.published
        ? '<span style="color:#22c55e;font-size:.75rem;font-weight:600">LIVE</span>'
        : '<span style="color:#f59e0b;font-size:.75rem;font-weight:600">DRAFT</span>';
      html += '<tr><td style="font-weight:500">' + (a.title||'') + '</td><td>' + pub + '</td><td>' + (a.created_at||'').slice(0,10) + '</td>'
        + '<td style="white-space:nowrap">'
        + '<button class="btn btn-outline btn-sm" onclick="openArticleEditor(' + a.id + ')" style="margin-right:.35rem">Edit</button>'
        + '<button class="btn btn-sm" style="background:' + (a.published?'#ef4444':'#22c55e') + ';color:#fff;margin-right:.35rem" onclick="toggleArticle(' + a.id + ')">' + (a.published?'Unpublish':'Publish') + '</button>'
        + (a.published ? '<a href="/articles/' + a.slug + '" target="_blank" class="btn btn-outline btn-sm" style="margin-right:.35rem">View</a>' : '')
        + '<button class="btn btn-sm" style="background:#374151;color:#fff" onclick="deleteArticle(' + a.id + ')">Del</button></td></tr>';
    });
    html += '</tbody></table></div>';
    el.innerHTML = html;
  } catch(e) { el.innerHTML = '<p style="color:#f87171">' + e.message + '</p>'; }
...
async function openArticleEditor(id) {
  editingArticleId = id;
  document.getElementById('articleOverlay').style.display = 'flex';
  if (!id) {
    ['artTitle','artSummary','artBody','artHeroKey'].forEach(function(x) { document.getElementById(x).value = ''; });
    document.getElementById('artPublished').checked = false;
    document.getElementById('artHeroPreview').innerHTML = '';
    document.getElementById('artEditorTitle').textContent = 'New Article';
    return;
  }
  try {
    var r = await fetch('/api/admin/articles/' + id); var d = await r.json(); var a = d.article;
    ...
    document.getElementById('artHeroKey').value = a.hero_image_r2_key||'';
    document.getElementById('artEditorTitle').textContent = 'Edit Article';
    updateHeroPreview();
...
function updateHeroPreview() {
  var k = document.getElementById('artHeroKey').value.trim();
  var p = document.getElementById('artHeroPreview');
  p.innerHTML = k ? '<img src="/media/serve/' + encodeURIComponent(k) + '" style="max-height:120px;border-radius:6px;margin-top:.5rem;display:block">' : '';
}
...
async function saveArticle() {
  var payload = {
    ...
    hero_image_r2_key: document.getElementById('artHeroKey').value.trim()
  };
  if (!payload.title) { alert('Title required'); return; }
  try {
    var r = await fetch('/api/admin/articles/' + (editingArticleId||'new'), {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload)});
    var d = await r.json();
    if (!d.ok) { alert('Save error: ' + (d.error||'unknown')); return; }
    closeArticleEditor(); loadArticles();
  } catch(e) { alert('Error: ' + e.message); }
}
window.saveArticle = saveArticle;

async function toggleArticle(id) {
  try { var r = await fetch('/api/admin/articles/' + id + '/toggle', {method:'POST'}); var d = await r.json(); if (d.ok) loadArticles(); }
  catch(e) { alert(e.message); }
}
...
async function deleteArticle(id) {
  ...
  try { var r = await fetch('/api/admin/articles/' + id, {method:'DELETE'}); var d = await r.json(); if (d.ok) loadArticles(); }
  catch(e) { alert(e.message); }
}
...
async function loadMedia() {
  var el = document.getElementById('mediaPanel');
  try {
    var r = await fetch('/api/media'); var d = await r.json();
    var html = '<div style="margin-bottom:1.25rem">'
      + '<div style="background:rgba(255,255,255,.04);border:2px dashed var(--border);border-radius:var(--r);padding:1.5rem;text-align:center;margin-bottom:1rem">'
      + '<div style="color:var(--muted);margin-bottom:.75rem;font-size:.85rem">Choose files to upload</div>'
      + '<select id="mediaCategory" style="background:rgba(255,255,255,.08);border:1px solid var(--border);color:#fff;padding:.4rem .75rem;border-radius:var(--r);font-size:.82rem;margin-right:.5rem">'
      + '<option value="article">Article Image</option><option value="hero">Hero Image</option><option value="project">Project Photo</option><option value="audio">Voice Note / Audio</option><option value="other">Other</option></select>'
      + '<label style="background:var(--a);color:#fff;padding:.45rem 1rem;border-radius:var(--r);cursor:pointer;font-size:.82rem;font-weight:600">Upload File'
      + '<input type="file" id="mediaFileInput" accept="image/*,video/*,audio/*,.pdf" style="display:none" onchange="uploadMedia()"/></label>'
      + '<div id="mediaUploadProgress" style="margin-top:.5rem;font-size:.78rem;color:var(--muted)"></div></div>';
    if (!d.media || !d.media.length) { el.innerHTML = html + '<p style="color:var(--muted)">No files uploaded yet.</p>'; return; }
    html += '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:1rem">';
    d.media.forEach(function(m) {
      var isImg = new RegExp('^image').test(m.content_type);
      var isAudio = new RegExp('^audio').test(m.content_type);
      var thumb = isImg ? '<img src="/media/serve/' + encodeURIComponent(m.r2_key) + '" style="width:100%;height:110px;object-fit:cover;border-radius:6px 6px 0 0;display:block"/>' : '';
      if (isAudio) thumb = '<div style="height:60px;display:flex;align-items:center;justify-content:center;font-size:1.8rem;background:rgba(200,168,75,.15);border-radius:6px 6px 0 0">&#127908;</div>';
      if (!thumb && !isAudio) thumb = '<div style="height:60px;display:flex;align-items:center;justify-content:center;font-size:1.5rem;background:rgba(255,255,255,.05);border-radius:6px 6px 0 0">&#128196;</div>';
      html += '<div style="background:var(--card);border:1px solid var(--border);border-radius:var(--r);overflow:hidden">' + thumb
        + '<div style="padding:.6rem">'
        + '<div style="font-size:.72rem;color:var(--muted);margin-bottom:.3rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + m.filename + '</div>'
        + '<div style="font-size:.68rem;color:var(--a);margin-bottom:.5rem">' + (m.category||'other') + '</div>'
        + (isAudio ? '<audio controls style="width:100%;margin-bottom:.5rem"><source src="/media/serve/' + encodeURIComponent(m.r2_key) + '" type="' + m.content_type + '"></audio>' : '')
        + '<div style="display:flex;gap:.35rem;flex-wrap:wrap">'
        + '<button class="btn btn-sm" style="background:rgba(255,255,255,.08);color:#fff;font-size:.68rem" data-key="' + m.r2_key + '" onclick="copyMediaKey(this.dataset.key)">Copy R2 Key</button>'
        + '<button class="btn btn-sm" style="background:#ef4444;color:#fff;font-size:.68rem" data-id="' + m.id + '" onclick="deleteMedia(this.dataset.id)">Del</button>'
        + '</div></div></div>';
    });
    html += '</div></div>';
    el.innerHTML = html;
...
async function uploadMedia() {
  var fi = document.getElementById('mediaFileInput');
  var cat = (document.getElementById('mediaCategory') || {}).value || 'other';
  var prog = document.getElementById('mediaUploadProgress');
  if (!fi || !fi.files.length) return;
  var file = fi.files[0];
  if (prog) prog.textContent = 'Uploading ' + file.name + '...';
  var fd = new FormData(); fd.append('file', file); fd.append('category', cat);
  try {
    var r = await fetch('/api/media/upload', {method:'POST', body:fd}); var d = await r.json();
    if (prog) prog.textContent = d.ok ? 'Uploaded!' : 'Error: ' + (d.error||'unknown');
    if (d.ok) setTimeout(function() { loadMedia(); }, 800);
...
window.copyMediaKey = copyMediaKey;

async function deleteMedia(id) {
  if (!confirm('Delete this file?')) return;
  try { var r = await fetch('/api/media/' + id, {method:'DELETE'}); var d = await r.json(); if (d.ok) loadMedia(); }
  catch(e) { alert(e.message); }
}
...
async function loadKnowledge() {
  var el = document.getElementById('knowledgePanel');
  try {
    var r = await fetch('/api/knowledge'); var d = await r.json();
    var html = '<div style="margin-bottom:1rem;display:flex;gap:.75rem;align-items:center">'
      + '<button class="btn btn-gold btn-sm" onclick="openSeedEditor(null)">+ Add Knowledge</button>'
      + '<span style="font-size:.8rem;color:var(--muted)">' + (d.seeds ? d.seeds.length : 0) + ' entries</span></div>';
    if (!d.seeds || !d.seeds.length) { el.innerHTML = html + '<p style="color:var(--muted)">No knowledge seeds yet.</p>'; return; }
    html += '<div style="display:grid;gap:.75rem">';
    d.seeds.forEach(function(s) {
      var emb = s.embedded ? '<span style="color:#22c55e;font-size:.72rem">Embedded</span>' : '<span style="color:#f59e0b;font-size:.72rem">Pending</span>';
      html += '<div style="background:rgba(255,255,255,.04);border:1px solid var(--border);border-radius:var(--r);padding:1rem">'
        + '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:1rem;margin-bottom:.4rem">'
        + '<div style="font-weight:600;color:#fff;font-size:.9rem">' + s.title + '</div>'
        + '<div style="display:flex;gap:.35rem;flex-shrink:0">' + emb
        + ' <button class="btn btn-outline btn-sm" data-id="' + s.id + '" onclick="openSeedEditor(this.dataset.id)">Edit</button>'
        + '<button class="btn btn-sm" style="background:#ef4444;color:#fff" data-id="' + s.id + '" onclick="deleteSeed(this.dataset.id)">Del</button></div></div>'
        + '<div style="font-size:.78rem;color:var(--muted)">' + s.body.slice(0,150) + (s.body.length>150?'...':'') + '</div>'
        + '<div style="font-size:.72rem;color:var(--a);margin-top:.35rem">' + (s.category||'general') + '</div></div>';
    });
    html += '</div>';
    el.innerHTML = html;
  } catch(e) { el.innerHTML = '<p style="color:#f87171">' + e.message + '</p>'; }
...
function openSeedEditor(id) {
  editingSeedId = id;
  document.getElementById('seedOverlay').style.display = 'flex';
  if (!id) {
    ['seedTitle','seedBody'].forEach(function(x) { document.getElementById(x).value = ''; });
    document.getElementById('seedCategory').value = 'general';
    document.getElementById('seedEditorTitle').textContent = 'Add Knowledge';
    return;
  }
  fetch('/api/knowledge').then(function(r) { return r.json(); }).then(function(d) {
    var s = (d.seeds||[]).find(function(x) { return x.id === id; });
    if (!s) return;
    document.getElementById('seedTitle').value = s.title||'';
    document.getElementById('seedBody').value = s.body||'';
    document.getElementById('seedCategory').value = s.category||'general';
    document.getElementById('seedEditorTitle').textContent = 'Edit Knowledge';
  });
}
window.openSeedEditor = openSeedEditor;

function closeSeedEditor() { document.getElementById('seedOverlay').style.display = 'none'; editingSeedId = null; }
window.closeSeedEditor = closeSeedEditor;

async function saveSeed() {
  var payload = { title: document.getElementById('seedTitle').value.trim(), body: document.getElementById('seedBody').value.trim(), category: document.getElementById('seedCategory').value };
  if (!payload.title || !payload.body) { alert('Title and content required'); return; }
  try {
    var r = await fetch('/api/knowledge/' + (editingSeedId||'new'), {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload)});
    var d = await r.json();
    if (!d.ok) { alert('Save error: ' + (d.error||'unknown')); return; }
    closeSeedEditor(); loadKnowledge();
  } catch(e) { alert('Error: ' + e.message); }
}
...
async function deleteSeed(id) {
  ...
  try { var r = await fetch('/api/knowledge/' + id, {method:'DELETE'}); var d = await r.json(); if (d.ok) loadKnowledge(); }
  catch(e) { alert(e.message); }
}
window.deleteSeed = deleteSeed;

// ── CMS Sections (Services / Projects / Reviews / Process / Contact) ──────────
var cmsData = {};

async function loadSection(section) {
  var el = document.getElementById('cmsPanel_' + section);
  if (!el) return;
  el.innerHTML = '<span class="spin"></span> Loading...';
  try {
    var r = await fetch('/api/content/' + section); var d = await r.json();
    cmsData[section] = d.data; renderCmsPanel(section, d.data);
  } catch(e) { el.innerHTML = '<p style="color:#f87171">' + e.message + '</p>'; }
}
window.loadSection = loadSection;

function renderCmsPanel(section, data) {
  var el = document.getElementById('cmsPanel_' + section);
  if (!el) return;
  if (section === 'contact') { renderContactPanel(el, data); return; }
  var html = '<div style="margin-bottom:1rem;display:flex;gap:.75rem;align-items:center;flex-wrap:wrap">'
    + '<button class="btn btn-gold btn-sm" onclick="addCmsItem(\'' + section + '\')">+ Add Item</button>'
    + '<button class="btn btn-outline btn-sm" onclick="saveCmsSection(\'' + section + '\')">Save Draft</button>'
    + '<button class="btn btn-sm" style="background:#22c55e;color:#fff" onclick="saveAndPublish(\'' + section + '\')">Save + Publish Live</button></div>';
  if (!data || !data.length) { el.innerHTML = html + '<p style="color:var(--muted)">No items yet.</p>'; return; }
  data.forEach(function(item, i) {
    html += '<div id="cmsItem_' + section + '_' + i + '" style="background:rgba(255,255,255,.04);border:1px solid var(--border);border-radius:var(--r);padding:1.25rem;margin-bottom:.75rem">'
      + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem">'
      + '<strong style="color:#fff">' + (item.title||item.tab||item.name||item.num||'Item#'+(i+1)) + '</strong>'
      + '<div style="display:flex;gap:.35rem">'
      + (i > 0 ? '<button class="btn btn-outline btn-sm" onclick="moveCmsItem(\'' + section + '\',' + i + ',-1)">&#8593;</button>' : '')
      + (i < data.length-1 ? '<button class="btn btn-outline btn-sm" onclick="moveCmsItem(\'' + section + '\',' + i + ',1)">&#8595;</button>' : '')
      + '<button class="btn btn-sm" style="background:#ef4444;color:#fff" onclick="deleteCmsItem(\'' + section + '\',' + i + ')">Del</button></div></div>'
      + renderCmsFields(section, item, i) + '</div>';
  });
  el.innerHTML = html;
}
window.renderCmsPanel = renderCmsPanel;

function imgFieldHtml(id, val) {
  var preview = val && val.startsWith('contractor-')
    ? '<img src="/media/serve/' + encodeURIComponent(val) + '" style="height:40px;width:60px;object-fit:cover;border-radius:4px">'
    : val && val.startsWith('http') ? '<img src="' + val + '" style="height:40px;width:60px;object-fit:cover;border-radius:4px">' : '';
  return '<label class="flabel">Image (R2 Key or URL)</label>'
    + '<div style="display:flex;gap:.5rem;align-items:center;margin-bottom:.85rem">'
    + '<input class="finput" id="' + id + '" value="' + (val||'') + '" placeholder="contractor-v003-2/media/..." style="margin-bottom:0"/>' + preview + '</div>';
}
window.imgFieldHtml = imgFieldHtml;

function renderCmsFields(section, item, i) {
  var html = ''; var p = 'cms_' + section + '_' + i + '_';
  function field(k, lbl, type) {
    var val = (item[k]||'').toString().replace(/"/g,'&quot;');
    if (type === 'img') return imgFieldHtml(p + k, item[k]||'');
    if (type === 'ta') return '<label class="flabel">' + lbl + '</label><textarea class="ftextarea" id="' + p+k + '" style="min-height:80px">' + val + '</textarea>';
    return '<label class="flabel">' + lbl + '</label><input class="finput" id="' + p+k + '" value="' + val + '" placeholder="' + lbl + '" style="margin-bottom:.65rem"/>';
  }
  if (section === 'services') {
    html += field('tab','Tab Label') + field('title','Title') + field('desc','Description','ta');
    html += '<label class="flabel">Highlights (one per line)</label><textarea class="ftextarea" id="' + p + 'highlights" style="min-height:80px">' + (item.highlights||[]).join('\n') + '</textarea>';
    html += imgFieldHtml(p + 'image_r2_key', item.image_r2_key||item.image_url||'');
  } else if (section === 'projects') {
    html += field('title','Project Title') + field('location','Location') + field('type','Type (for filter)') + field('desc','Description','ta');
    html += imgFieldHtml(p + 'image_r2_key', item.image_r2_key||item.image_url||'');
  } else if (section === 'reviews') {
    html += field('name','Reviewer Name') + field('project','Project & Location') + field('text','Review Text','ta');
    html += '<label class="flabel">Stars (1-5)</label><input class="finput" id="' + p + 'stars" type="number" min="1" max="5" value="' + (item.stars||5) + '" style="max-width:100px;margin-bottom:.65rem"/>';
  } else if (section === 'process') {
    html += field('num','Step Number (e.g. 01)') + field('title','Step Title') + field('desc','Step Description','ta');
  }
  return html;
}
window.renderCmsFields = renderCmsFields;

function renderContactPanel(el, data) {
  function field(k, lbl, type) {
    var val = (data[k]||'').toString().replace(/"/g,'&quot;');
    if (type === 'ta') return '<label class="flabel">' + lbl + '</label><textarea class="ftextarea" id="ct_' + k + '" style="min-height:70px">' + val + '</textarea>';
    return '<label class="flabel">' + lbl + '</label><input class="finput" id="ct_' + k + '" value="' + val + '" placeholder="' + lbl + '"/>';
  }
  var html = '<div style="display:flex;gap:.75rem;margin-bottom:1.25rem">'
    + '<button class="btn btn-outline btn-sm" onclick="saveCmsSection(\'contact\')">Save Draft</button>'
    + '<button class="btn btn-sm" style="background:#22c55e;color:#fff" onclick="saveAndPublish(\'contact\')">Save + Publish Live</button></div>'
    + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">'
    + field('company','Company Name') + field('phone','Phone Number') + field('phone_url','Phone URL (tel:+1...)') + field('license','License Number') + field('tagline','Trust Bar Tagline') + field('areas','Service Areas')
    + '</div>'
    + field('hero_title','Hero Headline') + field('hero_sub','Hero Subheading','ta')
    + '<label class="flabel">Hero Background Image (R2 key or URL)</label>'
    + '<div style="display:flex;gap:.5rem;align-items:center;margin-bottom:.85rem">'
    + '<input class="finput" id="ct_hero_image_r2_key" value="' + (data.hero_image_r2_key||data.hero_image_url||'') + '" placeholder="contractor-v003-2/media/..." style="margin-bottom:0"/>'
    + (data.hero_image_r2_key ? '<img src="/media/serve/' + encodeURIComponent(data.hero_image_r2_key) + '" style="height:40px;width:60px;object-fit:cover;border-radius:4px">' : '')
    + '</div>'
    + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">'
    + field('stat1_num','Stat 1 Number') + field('stat1_label','Stat 1 Label') + field('stat2_num','Stat 2 Number') + field('stat2_label','Stat 2 Label')
    + field('stat3_num','Stat 3 Number') + field('stat3_label','Stat 3 Label') + field('stat4_num','Stat 4 Number') + field('stat4_label','Stat 4 Label')
    + '</div>';
  el.innerHTML = html;
}
window.renderContactPanel = renderContactPanel;

function readCmsItem(section, item, i) {
  var p = 'cms_' + section + '_' + i + '_';
  function v(k) { var el = document.getElementById(p+k); return el ? el.value.trim() : ''; }
  var out = Object.assign({}, item);
  if (section === 'services') { out.tab=v('tab'); out.title=v('title'); out.desc=v('desc'); var hiEl=document.getElementById(p+'highlights'); out.highlights=hiEl?hiEl.value.trim().split('\n').map(function(h){return h.trim();}).filter(Boolean):item.highlights||[]; out.image_r2_key=v('image_r2_key'); }
  else if (section === 'projects') { out.title=v('title'); out.location=v('location'); out.type=v('type'); out.desc=v('desc'); out.image_r2_key=v('image_r2_key'); }
  else if (section === 'reviews') { out.name=v('name'); out.project=v('project'); out.text=v('text'); out.stars=parseInt(v('stars'))||5; }
  else if (section === 'process') { out.num=v('num'); out.title=v('title'); out.desc=v('desc'); }
  return out;
}
window.readCmsItem = readCmsItem;

function readContactData() {
  function v(k) { var el = document.getElementById('ct_'+k); return el ? el.value.trim() : ''; }
  return { company:v('company'), phone:v('phone'), phone_url:v('phone_url'), license:v('license'), tagline:v('tagline'), areas:v('areas'), hero_title:v('hero_title'), hero_sub:v('hero_sub'), hero_image_r2_key:v('hero_image_r2_key'), stat1_num:v('stat1_num'), stat1_label:v('stat1_label'), stat2_num:v('stat2_num'), stat2_label:v('stat2_label'), stat3_num:v('stat3_num'), stat3_label:v('stat3_label'), stat4_num:v('stat4_num'), stat4_label:v('stat4_label') };
}
window.readContactData = readContactData;

function getCurrentCmsData(section) { if (section === 'contact') return readContactData(); return (cmsData[section]||[]).map(function(item,i){return readCmsItem(section,item,i);}); }
window.getCurrentCmsData = getCurrentCmsData;

function addCmsItem(section) {
  var data = getCurrentCmsData(section); var id = Date.now().toString(36); var newItem = {id:id, sort:data.length+1};
  if (section==='services') newItem = Object.assign(newItem, {tab:'New Service',title:'',desc:'',highlights:[],image_url:'',image_r2_key:''});
  else if (section==='projects') newItem = Object.assign(newItem, {title:'New Project',location:'',type:'',desc:'',image_url:'',image_r2_key:''});
  else if (section==='reviews') newItem = Object.assign(newItem, {name:'',project:'',text:'',stars:5});
  else if (section==='process') newItem = Object.assign(newItem, {num:'0'+(data.length+1),title:'',desc:''});
  data.push(newItem); cmsData[section] = data; renderCmsPanel(section, data);
}
window.addCmsItem = addCmsItem;
...
function moveCmsItem(section, i, dir) { var data=getCurrentCmsData(section); var j=i+dir; if(j<0||j>=data.length)return; var tmp=data[i]; data[i]=data[j]; data[j]=tmp; cmsData[section]=data; renderCmsPanel(section,data); }
window.moveCmsItem = moveCmsItem;

async function saveCmsSection(section) {
  var data = getCurrentCmsData(section);
  try {
    var r = await fetch('/api/content/' + section, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({data:data})});
    var d = await r.json();
    if (d.ok) alert('Saved as draft. Click Publish Site Live on Dashboard to push live.');
    else alert('Save error: ' + (d.error||'unknown'));
  } catch(e) { alert('Error: ' + e.message); }
}
window.saveCmsSection = saveCmsSection;

async function saveAndPublish(section) {
  var data = getCurrentCmsData(section);
  try {
    var r1 = await fetch('/api/content/' + section, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({data:data})});
    var d1 = await r1.json(); if (!d1.ok) { alert('Save error: ' + (d1.error||'unknown')); return; }
    var r2 = await fetch('/api/publish', {method:'POST'}); var d2 = await r2.json();
    if (d2.ok) alert('Saved and published live!'); else alert('Saved but publish failed: ' + (d2.error||'unknown'));
  } catch(e) { alert('Error: ' + e.message); }
}
window.saveAndPublish = saveAndPublish;

--da51e519d358e3f697981b8b44dee441881779689b2c2a0379389b64c0a2
Content-Disposition: form-data; name="admin-login-wrapper.js"

var __defProp = Object.defineProperty;
var __name = (target, value) => __defProp(target, "name", { value, configurable: true });

// contractor-v004-demo.js
...
__name22(getContactConstants, "getContactConstants");
...
__name22(vecSearch, "vecSearch");
function csvCell(v) {
  return '"' + String(v == null ? "" : v).replace(/"/g, '""') + '"';
}
__name(csvCell, "csvCell");
__name2(csvCell, "csvCell");
__name22(csvCell, "csvCell");
...
__name22(validLeadStatus, "validLeadStatus");
...
__name22(validCallbackStatus, "validCallbackStatus");
...
__name22(loadContent, "loadContent");
...
function renderPublicHTML(content, articles) {
  ...
  const svcPanels = services.map((s, i) => {
    const hi = (s.highlights || []).map((h22) => "<li>" + esc(h22) + "</li>").join("");
    const src = imgSrc(s);
...
__name22(renderPublicHTML, "renderPublicHTML");
...
__name22(handlePublish, "handlePublish");
...
__name22(handleHome, "handleHome");
...
__name22(handleArticlesIndex, "handleArticlesIndex");
...
__name22(handlePublicArticlePage, "handlePublicArticlePage");
...
__name22(handleContentGet, "handleContentGet");
...
__name22(handleContentSave, "handleContentSave");
...
__name22(handleLeads, "handleLeads");
...
__name22(handleSeed, "handleSeed");
...
__name22(handleAdminLeads, "handleAdminLeads");
...
__name22(handleAdminLeadById, "handleAdminLeadById");
...
__name22(handleAdminCallbacks, "handleAdminCallbacks");
...
__name22(handleAdminCallbackById, "handleAdminCallbackById");
...
__name22(handleMediaUpload, "handleMediaUpload");
...
__name22(handleMediaList, "handleMediaList");
...
__name22(handleMediaDelete, "handleMediaDelete");
...
__name22(handleMediaAltText, "handleMediaAltText");
...
__name22(handleMediaServe, "handleMediaServe");
...
__name22(handleAdminArticles, "handleAdminArticles");
...
__name22(handleAdminArticleGet, "handleAdminArticleGet");
...
__name22(handleAdminArticleSave, "handleAdminArticleSave");
...
__name22(handleAdminArticleToggle, "handleAdminArticleToggle");
...
__name22(handleAdminArticleDelete, "handleAdminArticleDelete");
...
__name22(handleKnowledgeList, "handleKnowledgeList");
...
__name22(handleKnowledgeSave, "handleKnowledgeSave");
...
__name22(handleKnowledgeDelete, "handleKnowledgeDelete");
function buildAdmin() {
...
__name22(buildAdmin, "buildAdmin");
...
__name22(handleMemberMe, "handleMemberMe");
...
__name22(handleMemberSubmit, "handleMemberSubmit");
...
__name22(handleMemberSubmissions, "handleMemberSubmissions");
...
__name22(handleAdminMembers, "handleAdminMembers");
...
__name22(handleAdminSubmissions, "handleAdminSubmissions");
...
__name22(handleAdminSubmissionApprove, "handleAdminSubmissionApprove");
...
__name22(handleAdminSubmissionReject, "handleAdminSubmissionReject");
...
__name22(handleContributorPortal, "handleContributorPortal");
...
// content-hub.js
var CHAT_MODEL2 = "@cf/meta/llama-3.1-8b-instruct";
var R2_PREFIX2 = "contractor-v003-2/";
...
__name(esc2, "esc");
async function dbRun2(env, sql, p = []) {
  return env.V003_2_DB.prepare(sql).bind(...p).run();
}
__name(dbRun2, "dbRun");
async function dbAll2(env, sql, p = []) {
  const r = await env.V003_2_DB.prepare(sql).bind(...p).all();
  return r.results || [];
}
__name(dbAll2, "dbAll");
async function dbFirst2(env, sql, p = []) {
  return env.V003_2_DB.prepare(sql).bind(...p).first();
}
__name(dbFirst2, "dbFirst");
async function getMemberByToken(env, token) {
  try {
    const m = await dbFirst2(
      env,
...
__name(getMemberByToken, "getMemberByToken");
function renderCrewUploadPage(member, token, submitted = false, error = "") {
  const name = esc2(member ? member.name : "Team Member");
  const CSS = `
  ...
  `;
  if (submitted) {
    return h2(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0,viewport-fit=cover"><title>Submitted!</title><style>${CSS}</style></head><body><div class="card"><div class="logo">CCS<span>.</span></div><div class="greeting">Content Hub</div><div class="success"><h2>&#10003; Submitted!</h2><p>Your content is in the review queue. The admin will be notified.</p></div><br><button class="btn" onclick="location.reload()">Submit Another</button></div></body></html>`);
  }
  return h2(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0,viewport-fit=cover"><title>Content Upload</title><style>${CSS}</style></head><body>
<div class="card">
  <div class="logo">CCS<span>.</span></div>
  <div class="greeting">Hey ${name} &mdash; share a moment from today &#128247;</div>
  ${error ? `<div class="error">${esc2(error)}</div>` : ""}
  <form id="uploadForm">
...
async function submitContent(){
  ...
  fd.append('note',note);
  fd.append('token','${esc2(token)}');
  try{
    var r=await fetch('/crew/upload',{method:'POST',body:fd});
    var d=await r.json();
    if(d.ok){window.location.href='/crew/upload?token=${esc2(token)}&submitted=1';}
...
__name(generateCaptions, "generateCaptions");
    ...
    try {
      ...
      if (obj && obj.httpMetadata?.contentType?.startsWith("image/")) {
        ...
        if (blobRes.ok) {
          ...
          embed2 = { $type: "app.bsky.embed.images", images: [{ image: blobData.blob, alt: caption.slice(0, 100) }] };
        }
      }
    } catch (e) {
    }
...
async function handleContentHub(req, env) {
  const url = new URL(req.url);
  const path = url.pathname.replace(/\/+$/, "") || "/";
  ...
  if (method === "GET" && path === "/crew/upload") {
    ...
    if (!token) return h2(`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Access Denied</title></head><body style="font-family:system-ui;background:#0f1a2e;color:#e8eaf0;display:flex;align-items:center;justify-content:center;height:100vh;text-align:center;padding:2rem"><div><h1 style="color:#ef4444;font-size:1.5rem;margin-bottom:.75rem">&#128274; Invalid Link</h1><p style="color:#8899aa">This upload link is invalid or has expired.<br>Ask your manager for a new link.</p></div></body></html>`);
    const member = await getMemberByToken(env, token);
    if (!member) return h2(`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Access Denied</title></head><body style="font-family:system-ui;background:#0f1a2e;color:#e8eaf0;display:flex;align-items:center;justify-content:center;height:100vh;text-align:center;padding:2rem"><div><h1 style="color:#ef4444;font-size:1.5rem;margin-bottom:.75rem">&#128274; Invalid Link</h1><p style="color:#8899aa">This upload link is invalid or has expired.<br>Ask your manager for a new link.</p></div></body></html>`);
  ...
  if (method === "POST" && path === "/crew/upload") {
    try {
      ...
      const note = (fd.get("note") || "").trim().slice(0, 1e3);
      const file = fd.get("file");
      const member = await getMemberByToken(env, token);
      if (!member) return j2({ ok: false, error: "Invalid token" }, 403);
      let r2Key = null;
      let contentType = null;
      if (file && file.size > 0) {
        const ext = (file.name.split(".").pop() || "").toLowerCase();
        const allowed = ["jpg", "jpeg", "png", "gif", "webp", "mp4", "mov", "heic", "m4v"];
        if (!allowed.includes(ext)) return j2({ ok: false, error: "File type not allowed" }, 400);
        if (file.size > 50 * 1024 * 1024) return j2({ ok: false, error: "File too large (50MB max)" }, 400);
        const key = R2_PREFIX2 + "content-hub/" + Date.now() + "-" + uid2() + "." + ext;
        const buf = await file.arrayBuffer();
        await env.V003_2_R2.put(key, buf, { httpMetadata: { contentType: file.type } });
        r2Key = key;
        contentType = file.type;
      }
      if (!r2Key && !note) return j2({ ok: false, error: "Please add a photo/video or a note" }, 400);
      await dbRun2(
        env,
        `INSERT INTO content_submissions (submitted_by, raw_note, r2_key, content_type, status, created_at) VALUES (?,?,?,?,?,?)`,
        [member.name || member.id, note, r2Key, contentType, "pending", now2()]
      );
      return j2({ ok: true, message: "Submitted! Your content is in the review queue." });
    } catch (e) {
      return j2({ ok: false, error: e.message }, 500);
    }
  }
  if (method === "GET" && path === "/api/admin/content-submissions") {
    const status = url.searchParams.get("status") || "pending";
    const sql = status === "all" ? "SELECT * FROM content_submissions ORDER BY id DESC LIMIT 200" : "SELECT * FROM content_submissions WHERE status=? ORDER BY id DESC LIMIT 200";
    const params = status === "all" ? [] : [status];
    const rows = await dbAll2(env, sql, params);
    return j2({ ok: true, submissions: rows });
  }
  const genMatch = path.match(/^\/api\/admin\/content-submissions\/(\d+)\/generate-captions$/);
  if (method === "POST" && genMatch) {
    const id = genMatch[1];
    const row = await dbFirst2(env, "SELECT * FROM content_submissions WHERE id=?", [id]);
    if (!row) return j2({ ok: false, error: "Not found" }, 404);
    let coName = "CCS Services Group";
    try {
      const ct = await dbFirst2(env, "SELECT data FROM site_content WHERE section='contact'");
      if (ct) coName = JSON.parse(ct.data).company || coName;
    } catch (e) {
    }
    const note = row.raw_note || "Great work by our team today";
    const captions = await generateCaptions(env, note, coName);
    await dbRun2(env, "UPDATE content_submissions SET ai_captions=? WHERE id=?", [JSON.stringify(captions), id]);
    return j2({ ok: true, captions });
  }
  ...
  if (method === "POST" && approveMatch) {
    ...
    await dbRun2(
      ...
      ["approved", caption, JSON.stringify(platforms), now2(), id]
    );
    return j2({ ok: true, id, status: "approved" });
  }
  const rejectMatch = path.match(/^\/api\/admin\/content-submissions\/(\d+)\/reject$/);
  if (method === "POST" && rejectMatch) {
    const id = rejectMatch[1];
    await dbRun2(env, "UPDATE content_submissions SET status=?, reviewed_at=? WHERE id=?", ["rejected", now2(), id]);
    return j2({ ok: true, id, status: "rejected" });
  }
  ...
  if (method === "POST" && postMatch) {
    ...
    if (platform === "bluesky") {
      ...
      return j2({ ok: false, error: "Unknown platform: " + platform }, 400);
    }
    await dbRun2(
      env,
      "INSERT INTO social_posts (submission_id, platform, status, post_url, caption_used, error_message, posted_at) VALUES (?,?,?,?,?,?,?)",
      [id, platform, status, postUrl, caption, errorMsg, now2()]
    );
    if (status === "posted") {
      await dbRun2(env, "UPDATE content_submissions SET status=? WHERE id=?", ["posted", id]);
    }
    return j2({ ok: status !== "failed", status, post_url: postUrl, error: errorMsg, platform });
  }
  if (method === "GET" && path === "/api/admin/social-posts") {
    const rows = await dbAll2(env, "SELECT * FROM social_posts ORDER BY id DESC LIMIT 200");
    return j2({ ok: true, posts: rows });
  }
  return null;
}
__name(handleContentHub, "handleContentHub");

// admin-login-wrapper.js
import ADMIN_FIX_JS from "./92d7cbaa12be3419dec4eb6f107d9c333668b068-admin-fix.txt";
...
async function fastStatus(env) {
  ...
  try {
    const r = await env.V003_2_DB.prepare("SELECT COUNT(*) AS c FROM leads").first();
    out.leads = r?.c || 0;
    out.db = true;
  } catch (e) {
  }
  try {
    const r = await env.V003_2_DB.prepare("SELECT COUNT(*) AS c FROM articles").first();
    out.articles = r?.c || 0;
  } catch (e) {
  }
  try {
    const r = await env.V003_2_DB.prepare("SELECT COUNT(*) AS c FROM callbacks").first();
    out.callbacks = r?.c || 0;
  } catch (e) {
  }
  try {
    const r = await env.V003_2_DB.prepare("SELECT published_at FROM site_snapshot LIMIT 1").first();
    out.has_snapshot = !!r;
  } catch (e) {
  }
  try {
...
__name(patchAdminHtml, "patchAdminHtml");
var admin_login_wrapper_default = {
  async fetch(request, env, ctx) {
    const url = new URL(request.url);
    const path = url.pathname.replace(/\/+$/, "") || "/";
    ...
    if (request.method === "GET" && path === "/admin") {
      const response = await contractor_v003_2_afo_default.fetch(request, env, ctx);
    ...
    return contractor_v003_2_afo_default.fetch(request, env, ctx);
  }
};
export {
  admin_login_wrapper_default as default
};
//# sourceMappingURL=admin-login-wrapper.js.map